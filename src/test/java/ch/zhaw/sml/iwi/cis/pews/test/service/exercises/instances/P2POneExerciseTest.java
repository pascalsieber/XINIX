...
@RunWith( OrderedRunner.class ) public class P2POneExerciseTest
{
	...
	@TestOrder( order = 4 ) @Test public void testSetOutput() throws IOException
	{
		...
		List<P2POneData> stored = TestUtil.objectMapper.readValue( TestUtil.objectMapper.writeValueAsString( data ),
				TestUtil.makeCollectionType( P2POneData.class ) );

		assertTrue( stored.size() == 1 );
		for ( P2POneData d : stored )
		{
			assertTrue( d.getOwner().getID().equals( owner.getID() ) );
			for ( P2POneKeyword keyword : d.getKeywords() )
			{
				assertTrue( keyword.getKeyword().equals( outputOne ) || keyword.getKeyword().equals( outputTwo ) );
			}
		}
	}
	...
	@TestOrder( order = 5 ) @Test public void testGetOutput() throws IOException
	{
		...
		// check specifics
		List<String> dataKeywordIds = new ArrayList<String>();
		for ( ExerciseDataImpl d : comparableData )
		{
			dataKeywordIds.addAll( TestUtil.extractIds( ( (P2POneData)d ).getKeywords() ) );
		}

		List<String> outputKeywordIds = new ArrayList<String>();
		for ( ExerciseDataImpl o : comparableOutput )
		{
			outputKeywordIds.addAll( TestUtil.extractIds( ( (P2POneData)o ).getKeywords() ) );
		}
