...
public class ZhawEngine implements LifecycleObject
{
	...
	private static void setupEntityManager()
	{
		...
		registry.registerManagedObjectType( new EntityManagerFactoryLifecycleManager( "pews" ),
				"pewsFactory",
				Scope.CLASSLOADER );
		EntityManagerLifecycleManager lifecycleManager = new EntityManagerLifecycleManager( (EntityManagerFactory)registry
				.getManagedObject( "pewsFactory" ) );
	...
	{
		serverControl = NetworkServerControlWrapper.__new( InetAddressWrapper.getByName( "0.0.0.0" ),
				PewsConfig.getDerbyPort() );
	...
	private static ServletContextHandler getServletContextHandler()
	{
		...
		holder.getRegistration()
				.setMultipartConfig( new MultipartConfigElement( "data/tmp", 524288000, 524288000, 262144 ) );
		ServletContextHandler handler = new ServletContextHandler();
		handler.setContextPath( IdentifiableObjectRestService.SERVICES_BASE );
		handler.addServlet( holder, "/*" );
		handler.addFilter( ServletContextFilter.class,
				"/*",
	...
	private static void configureRootUser()
	{
		...
		UserImpl user = new UserImpl( new PasswordCredentialImpl( "root" ),
	...
	@SuppressWarnings( "unused" ) private static void configureSampleWorkshop()
	{
		UserService userService = getManagedObjectRegistry().getManagedObject( UserServiceImpl.class.getSimpleName() );
		RoleService roleService = getManagedObjectRegistry().getManagedObject( RoleServiceImpl.class.getSimpleName() );
		SessionService sessionService = getManagedObjectRegistry().getManagedObject( SessionServiceImpl.class.getSimpleName() );
		WorkshopTemplateService workshopTemplateService = getManagedObjectRegistry().getManagedObject(
				WorkshopTemplateServiceImpl.class.getSimpleName() );
		WorkshopService workshopService = getManagedObjectRegistry().getManagedObject( WorkshopServiceImpl.class.getSimpleName() );
		ExerciseTemplateService exerciseTemplateService = getManagedObjectRegistry().getManagedObject(
				ExerciseTemplateServiceImpl.class.getSimpleName() );
		ExerciseService exerciseService = getManagedObjectRegistry().getManagedObject( ExerciseServiceImpl.class.getSimpleName() );
		ExerciseDataService exerciseDataService = getManagedObjectRegistry().getManagedObject( ExerciseDataServiceImpl.class
				.getSimpleName() );
		InvitationService invitationService = getManagedObjectRegistry().getManagedObject( InvitationServiceImpl.class.getSimpleName() );
		XinixImageMatrixService xinixImageMatrixService = getManagedObjectRegistry().getManagedObject(
				XinixImageMatrixServiceImpl.class.getSimpleName() );
		...
		String wsTemplateID = workshopTemplateService.persist( new PinkElefantTemplate( rootUser,
		...
		String wsID = workshopService.persist( new PinkElefantWorkshop( "p.i.n.k.elefant Workshop",
				"Beispiel eines p.i.n.k.elefant Workshops",
				(PinkElefantTemplate)workshopTemplateService.findWorkshopTemplateByID( wsTemplateID ) ) );

		// workshop start (poster template)
		String startTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				180,
				false,
		...
		String endTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				180,
				false,
		...
		// pinklabs template
		String pinklabsTemplateID = exerciseTemplateService.persist( new PinkLabsTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
		...
		// p2pOne template
		String p2poneTemplateID = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
				...
				"Post2Paper 1",
				"Post2Paper 1 Tool",
				PewsConfig.getImageDir() + "/business.jpg" ) );

		// p2ptwo template
		String p2pTwoTemplateID = exerciseTemplateService.persist( new P2PTwoTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				180,
				true,
		...
		for ( String url : imageUrls )
		{
			try
			{
				...
				XINIX_IMAGES.add( (MediaObject)mediaService.findByID( mediaService.persist( new MediaObject( "image/png",
		...
		// xinix template
		String xinixTemplateID = exerciseTemplateService.persist( new XinixTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				60,
				false,
		...
		// you2me template
		String you2meTemplateID = exerciseTemplateService.persist( new You2MeTemplate( rootUser,
				true,
				TimeUnit.MINUTES,
				2,
				true,
				...
				"You2Me",
				"You2Me Tool",
				new HashSet<>( Arrays.asList( "Was möchtests Du gerne lernen?",
						"Wie kannst DU dies Deinem Gegenüber beibringen?" ) ) ) );

		// simply prototyping template
		String simplyprotoTemplateID = exerciseTemplateService.persist( new SimplyPrototypingTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				300,
				true,
		...
		String compressionTemplateID = exerciseTemplateService.persist( new CompressionTemplate( rootUser,
				true,
				TimeUnit.MINUTES,
				20,
				true,
				...
				"Kompression",
				"Kompression Tool",
				Arrays.asList( "Produkteigenschaften", "Werbung", "Vertrieb" ) ) );

		// evaluation template
		String evaluationTemplateID = exerciseTemplateService.persist( new EvaluationTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				60,
				false,
		...
		String evaluationResultTemplateID = exerciseTemplateService.persist( new EvaluationResultTemplate( rootUser,
				false,
				TimeUnit.MINUTES,
				10,
				false,
		...
		exerciseService.persist( new PosterExercise( "Start",
				"p.i.n.k.elefant Start",
				(PosterTemplate)exerciseTemplateService.findByID( startTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String pinklabsExID = exerciseService.persist( new PinkLabsExercise( "p.i.n.k.labs",
				"p.i.n.k.labs Tool",
				(PinkLabsTemplate)exerciseTemplateService.findByID( pinklabsTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String p2poneExID = exerciseService.persist( new P2POneExercise( "Post2Paper 1",
				"Post2Paper 1 Tool",
				(P2POneTemplate)exerciseTemplateService.findByID( p2poneTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String you2meExID = exerciseService.persist( new You2MeExercise( "You2me",
				"You2me Tool",
				(You2MeTemplate)exerciseTemplateService.findByID( you2meTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String p2ptwoExID = exerciseService.persist( new P2PTwoExercise( "Post2Paper 2",
				"Post2Paper 2 Tool",
				(P2PTwoTemplate)exerciseTemplateService.findByID( p2pTwoTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String xinixExID = exerciseService.persist( new XinixExercise( "XINIX",
				"XINIX-Tool",
				(XinixTemplate)exerciseTemplateService.findByID( xinixTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String simplyPrototypingExID = exerciseService.persist( new SimplyPrototypingExercise( "Simply Prototyping",
				"Simply Prototyping Tool",
				(SimplyPrototypingTemplate)exerciseTemplateService.findByID( simplyprotoTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String compressionExID = exerciseService.persist( new CompressionExercise( "Kompression",
				"Kompression Tool",
				(CompressionTemplate)exerciseTemplateService.findByID( compressionTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		String evaluationExID = exerciseService.persist( new EvaluationExercise( "Evaluation",
				"Evaluation Tool",
				(EvaluationTemplate)exerciseTemplateService.findByID( evaluationTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new EvaluationResultExercise( "Auswertung der Evaluation",
				"Tool zur Auswertung der Evaluation",
				(EvaluationResultTemplate)exerciseTemplateService.findByID( evaluationResultTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Ende",
				"p.i.n.k.elefant Workshop Ende",
				(PosterTemplate)exerciseTemplateService.findByID( endTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );

		// sample exercise data

		// pinklabs data
		exerciseDataService.persist( new PinkLabsExerciseData( rootUser,
				(WorkflowElementImpl)exerciseService.findByID( pinklabsExID ),
				Arrays.asList( "Internet", "Zeitungen" ) ) );

		// p2pone data
		String p2pOneDataID = exerciseDataService.persist( new P2POneData( rootUser,
				(WorkflowElementImpl)exerciseService.findByID( p2poneExID ),
				Arrays.asList( "Spass", "Spontan" ) ) );

		// you2me data
		exerciseDataService.persist( new You2MeExerciseData( rootUser,
				(WorkflowElementImpl)exerciseService.findByID( you2meExID ),
				Arrays.asList( new DialogEntry( DialogRole.RoleA, "Englisch" ),
		...
		exerciseDataService.persist( new P2PTwoData( rootUser,
		...
		exerciseDataService.persist( new XinixData( rootUser,
				(WorkflowElementImpl)exerciseService.findByID( xinixExID ),
				associations,
				XINIX_IMAGES.iterator().next() ) );
		...
		try
		{
			...
			FileUtils.copyURLToFile(
					new URL(
							"http://91ef69bade70f992a001-b6054e05bb416c4c4b6f3b0ef3e0f71d.r93.cf3.rackcdn.com/laptop-with-business-chart-10032918.jpg" ),
					file );
			byte[] blob = IOUtils.toByteArray( new FileInputStream( file ) );
			String mediaObjectID = mediaService.persist( new MediaObject( "image/jpeg",
					blob,
					MediaObjectType.SIMPLYPROTOTYPING ) );

			exerciseDataService.persist( new SimplePrototypingData( rootUser,
					(WorkflowElementImpl)exerciseService.findByID( simplyPrototypingExID ),
					(MediaObject)mediaService.findByID( mediaObjectID ) ) );
		}
		catch ( IOException e )
		{
		...
		// compression data
		CompressionExerciseData compressionData = exerciseDataService.findByID( exerciseDataService.persist( new CompressionExerciseData( rootUser,
				(WorkflowElementImpl)exerciseService.findByID( compressionExID ),
				Arrays.asList( new CompressionExerciseDataElement( "Werbekampagne auf Youtube",
								"Werbekampagne auf Youtube wird unserem Produkt helfen." ),
						new CompressionExerciseDataElement( "Werbekampagne auf Facebook",
								"Werbekampagne auf Facebook wird unserem Produkt helfen." ) ) ) ) );

		// evaluation data
		exerciseDataService.persist( new EvaluationExerciseData( rootUser,
				(WorkflowElementImpl)exerciseService.findByID( evaluationExID ),
				new Evaluation( rootUser, compressionData.getSolutions().get( 0 ), new Score( rootUser, 4 ) ) ) );

		// session
		String sessionID = sessionService.persist( new SessionImpl( "Beispiel Session",
		...
		invitationService.persist( new Invitation( rootUser,
				rootUser,
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		...
		String participantID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
		...
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participantID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// configure third user to join session (as participant)
		String participant2ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participating 2",
				"participant 2",
		...
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participant2ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		...
		String secondSessionID = sessionService.persist( new SessionImpl( "Zweite Beispiel Session",
				...
				null,
				null ) );
		invitationService.persist( new Invitation( null,
				(UserImpl)userService.findByID( rootUser.getID() ),
				(SessionImpl)sessionService.findByID( secondSessionID ) ) );

		sessionService.start( sessionID );

		// add some invitations
		invitationService.persist( new Invitation( (UserImpl)userService.findByID( rootUser.getID() ),
				(UserImpl)userService.findByID( rootUser.getID() ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		invitationService.persist( new Invitation( (UserImpl)userService.findByID( rootUser.getID() ),
				(UserImpl)userService.findByID( participant2ID ),
	...
	@SuppressWarnings( "unused" ) private static void configureDemoWorkshop()
	{
		UserService userService = getManagedObjectRegistry().getManagedObject( UserServiceImpl.class.getSimpleName() );
		RoleService roleService = getManagedObjectRegistry().getManagedObject( RoleServiceImpl.class.getSimpleName() );
		SessionService sessionService = getManagedObjectRegistry().getManagedObject( SessionServiceImpl.class.getSimpleName() );
		...
		// sample workshop instance
		String wsID = workshopService.persist( new PinkElefantWorkshop( "p.i.n.k.elefant Workshop",
				"Demo p.i.n.k.elefant Workshops",
				(PinkElefantTemplate)workshopTemplateService.findByID( wsTemplateID ) ) );
		...
		String you2meTemplateID = exerciseTemplateService.persist( new You2MeTemplate( rootUser,
				...
				"You2me",
				"You2me Tool",
				new HashSet<String>( Arrays.asList( "Was möchtests Du gerne lernen?",
						"Wie kannst DU dies Deinem Gegenüber beibringen?" ) ) ) );
		...
		// session
		String sessionID = sessionService.persist( new SessionImpl( "Demo Session",
		...
		// executor
		String executorID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				...
				"executor" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( executorID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// make second async session demo1
		String asyncSessionID1 = sessionService.persist( new SessionImpl( "Demo Session ASYNC",
		...
		// demo1
		String demo1ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 1",
				"@demo",
				"demo1" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( demo1ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// demo2
		String demo2ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 2",
				"@demo",
				"demo2" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( demo2ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// demo3
		String demo3ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 3",
				"@demo",
				"demo3" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( demo3ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// demo4
		String demo4ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 4",
				"@demo",
				"demo4" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( demo4ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// demo5
		String demo5ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 5",
				"@demo",
				"demo5" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( demo5ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// demo6
		String demo6ID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 6",
				"@demo",
				"demo6" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( demo6ID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );

		// async executor
		String asyncExecutorID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( EXECUTER_ROLE_ID ),
				null,
				"async executor",
				"@demo",
				"asyncexecutor" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( asyncExecutorID ),
				(SessionImpl)sessionService.findByID( asyncSessionID1 ) ) );

		// async user
		String asyncUserID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participant 7",
				"@demo",
				"async" ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( asyncUserID ),
				(SessionImpl)sessionService.findByID( asyncSessionID1 ) ) );

		// invitations, in case we show user profile
		invitationService.persist( new Invitation( demoRootUser,
				(UserImpl)userService.findByID( executorID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
	...
	@SuppressWarnings( "unused" ) private static void configurePostWorkshop()
	{
		RoleService roleService = getManagedObjectRegistry().getManagedObject( RoleServiceImpl.class.getSimpleName() );
		UserService userService = getManagedObjectRegistry().getManagedObject( UserServiceImpl.class.getSimpleName() );
		ClientService clientService = getManagedObjectRegistry().getManagedObject( ClientServiceImpl.class.getSimpleName() );
		...
		String wsTemplateID = workshopTemplateService.persist( new PinkElefantTemplate( postRootUser,
		...
		// workshop instance
		String wsID = workshopService.persist( new PinkElefantWorkshop( "Post Workshop",
				"p.i.n.k.elefant Workshop mit der Post",
				(PinkElefantTemplate)workshopTemplateService.findWorkshopTemplateByID( wsTemplateID ) ) );

		// pinklabs template 1
		String pinklabsTemplateID1 = exerciseTemplateService.persist( new PinkLabsTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
		...
		// pinklabs template 2
		String pinklabsTemplateID2 = exerciseTemplateService.persist( new PinkLabsTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
		...
		// pinklabs template 3
		String pinklabsTemplateID3 = exerciseTemplateService.persist( new PinkLabsTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
		...
		// pinklabs template 4
		String pinklabsTemplateID4 = exerciseTemplateService.persist( new PinkLabsTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
		...
		// p2p one iteration 1
		String p2poneTemplateID1 = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
				...
				"Post2Paper 1 (1/4)",
				"Post2Paper 1 Tool 1",
				PewsConfig.getImageDir() + "/single.jpg" ) );

		// p2p one iteration 2
		String p2poneTemplateID2 = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
				...
				"Post2Paper 1 (2/4)",
				"Post2Paper 1 Tool 2",
				PewsConfig.getImageDir() + "/empfangsdame.jpg" ) );

		// p2p one iteration 3
		String p2poneTemplateID3 = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
				...
				"Post2Paper 1 (3/4)",
				"Post2Paper 1 Tool 3",
				PewsConfig.getImageDir() + "/hausfrau.jpg" ) );

		// p2p one iteration 4
		String p2poneTemplateID4 = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				120,
				true,
				...
				"Post2Paper 1 (4/4)",
				"Post2Paper 1 Tool 4",
				PewsConfig.getImageDir() + "/senioren.jpg" ) );
		...
		String xinixTemplateID1 = exerciseTemplateService.persist( new XinixTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				180,
				false,
		...
		String xinixTemplateID2 = exerciseTemplateService.persist( new XinixTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				180,
				false,
		...
		String xinixTemplateID3 = exerciseTemplateService.persist( new XinixTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				180,
				false,
		...
		String xinixTemplateID4 = exerciseTemplateService.persist( new XinixTemplate( rootUser,
				true,
				TimeUnit.SECONDS,
				180,
				false,
		...
		String evaluationTemplateID = exerciseTemplateService.persist( new EvaluationTemplate( rootUser,
				...
				"Bewertungsrunde",
				"Tool zur Evaluation der erarbeiteten Lösungen",
				5 ) );
		...
		String introTemplateID1 = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				120,
				false,
		...
		String introTemplateID2 = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				30,
				false,
		...
		String pinklabsIntroTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				15,
				false,
		...
		String p2pOneIntroTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				15,
				false,
		...
		String p2pTwoIntroTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				15,
				false,
		...
		String xinixIntroTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				15,
				false,
		...
		String simplyprotoIntroTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				120,
				false,
		...
		String introTemplateID3 = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				30,
				false,
		...
		String introTemplateID4 = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				30,
				false,
		...
		String outroTemplateID = exerciseTemplateService.persist( new PosterTemplate( rootUser,
				false,
				TimeUnit.SECONDS,
				120,
				false,
		...
		// exercise instances
		exerciseService.persist( new PosterExercise( "Begrüssung",
				"Workshop Start Tool",
				(PosterTemplate)exerciseTemplateService.findByID( introTemplateID1 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Intro",
				"Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( introTemplateID2 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "p.i.n.k.labs Intro",
				"p.i.n.k.labs Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( pinklabsIntroTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PinkLabsExercise( "p.i.n.k.labs (1/4)",
				"p.i.n.k.labs Tool 1",
				(PinkLabsTemplate)exerciseTemplateService.findByID( pinklabsTemplateID1 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PinkLabsExercise( "p.i.n.k.labs (2/4)",
				"p.i.n.k.labs Tool 2",
				(PinkLabsTemplate)exerciseTemplateService.findByID( pinklabsTemplateID2 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PinkLabsExercise( "p.i.n.k.labs (3/4)",
				"p.i.n.k.labs Tool 3",
				(PinkLabsTemplate)exerciseTemplateService.findByID( pinklabsTemplateID3 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PinkLabsExercise( "p.i.n.k.labs (4/4)",
				"p.i.n.k.labs Tool 4",
				(PinkLabsTemplate)exerciseTemplateService.findByID( pinklabsTemplateID4 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Post2Paper 1 Intro",
				"Post2Paper 1 Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( p2pOneIntroTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new P2POneExercise( "Post2Paper 1 (1/4)",
				"Post2Paper 1 Tool 1",
				(P2POneTemplate)exerciseTemplateService.findByID( p2poneTemplateID1 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new P2POneExercise( "Post2Paper 1 (2/4)",
				"Post2Paper 1 Tool 2",
				(P2POneTemplate)exerciseTemplateService.findByID( p2poneTemplateID2 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new P2POneExercise( "Post2Paper 1 (3/4)",
				"Post2Paper 1 Tool 3",
				(P2POneTemplate)exerciseTemplateService.findByID( p2poneTemplateID3 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new P2POneExercise( "Post2Paper 1 (4/4)",
				"Post2Paper 1 Tool 4",
				(P2POneTemplate)exerciseTemplateService.findByID( p2poneTemplateID4 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Post2Paper 2 Intro",
				"Post2Paper 2 Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( p2pTwoIntroTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new P2PTwoExercise( "Post2Paper 2",
				"Post2Paper 2 Tool",
				(P2PTwoTemplate)exerciseTemplateService.findByID( p2pTwoTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "XINIX-Tool Intro",
				"XINIX-Tool Intro",
				(PosterTemplate)exerciseTemplateService.findByID( xinixIntroTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new XinixExercise( "XINIX-Tool (1/4)",
				"XINIX-Tool 1",
				(XinixTemplate)exerciseTemplateService.findByID( xinixTemplateID1 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new XinixExercise( "XINIX-Tool (2/4)",
				"XINIX-Tool 2",
				(XinixTemplate)exerciseTemplateService.findByID( xinixTemplateID2 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new XinixExercise( "XINIX-Tool (3/4)",
				"XINIX-Tool 3",
				(XinixTemplate)exerciseTemplateService.findByID( xinixTemplateID3 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new XinixExercise( "XINIX-Tool (4/4)",
				"XINIX-Tool 4",
				(XinixTemplate)exerciseTemplateService.findByID( xinixTemplateID4 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Simply Prototyping Intro",
				"Simply Prototyping Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( simplyprotoIntroTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new SimplyPrototypingExercise( "Simply Prototyping",
				"Simply Prototyping Tool",
				(SimplyPrototypingTemplate)exerciseTemplateService.findByID( simplyprotoTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Kompression Intro",
				"Kompression Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( introTemplateID3 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new CompressionExercise( "Kompression",
				"Kompression Tool",
				(CompressionTemplate)exerciseTemplateService.findByID( compressionTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Bewertung Intro",
				"Bewertung Intro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( introTemplateID4 ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new EvaluationExercise( "Bewertungsrunde",
				"Tool zur Evaluation der erarbeiteten Lösungen",
				(EvaluationTemplate)exerciseTemplateService.findByID( evaluationTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new EvaluationResultExercise( "Resultate der Bewertungsrunde",
				"Tool zur Auswertung der Resultate aus der Bewertungsrunde",
				(EvaluationResultTemplate)exerciseTemplateService.findByID( evaluationResultTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Outro Intro",
				"Outro Tool",
				(PosterTemplate)exerciseTemplateService.findByID( outroTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		exerciseService.persist( new PosterExercise( "Abschluss",
				"Workshop End Tool",
				(PosterTemplate)exerciseTemplateService.findByID( endTemplateID ),
				workshopService.findWorkshopByID( wsID ) ) );
		...
		String participantID1 = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participating",
				"participant 1",
		...
		String participantID2 = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participating",
				"participant 2",
		...
		String participantID3 = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participating",
				"participant 3",
		...
		String participantID4 = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participating",
				"participant 4",
		...
		String participantID5 = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"participating",
				"participant 5",
		...
		String executerID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc123" ),
				(RoleImpl)roleService.findByID( EXECUTER_ROLE_ID ),
				null,
				"executing",
				"executer",
		...
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participantID1 ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participantID2 ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participantID3 ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participantID4 ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( participantID5 ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( executerID ),
				(SessionImpl)sessionService.findByID( sessionID ) ) );
	...
	private static void configureSBBWorkshop()
	{
		...
		// workshop template (pinkelefant)
		String wsTemplateID = workshopTemplateService.persist( new PinkElefantTemplate( sbbRootUser,
		...
		// workshop instance
		String wsID = workshopService.persist( new PinkElefantWorkshop( "SBB Workshop",
				"p.i.n.k.elefant Workshop mit der SBB",
				(PinkElefantTemplate)workshopTemplateService.findByID( wsTemplateID ) ) );
		...
		String p2poneTemplateID1 = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				...
				"Post2Paper 1 (1/4)",
				"Post2Paper 1 Tool 1",
				PewsConfig.getImageDir() + "/familie.jpg" ) );
		...
		String p2poneTemplateID4 = exerciseTemplateService.persist( new P2POneTemplate( rootUser,
				...
				"Post2Paper 1 (4/4)",
				"Post2Paper 1 Tool 4",
				PewsConfig.getImageDir() + "/schueler.jpg" ) );
		...
		// synchronous session
		String syncSessionID = sessionService.persist( new SessionImpl( "Synchrone SBB Session",
		...
		String syncParticipantID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"Teilnehmer",
				"SBB Synchron",
		...
		String syncExecutorID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( EXECUTER_ROLE_ID ),
				null,
				"Executor",
				"SBB Synchron",
		...
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( syncParticipantID ),
				(SessionImpl)sessionService.findByID( syncSessionID ) ) );
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( syncExecutorID ),
				(SessionImpl)sessionService.findByID( syncSessionID ) ) );

		// asynchronous session
		String asyncSessionID = sessionService.persist( new SessionImpl( "Asynchrone SBB Session",
		...
		String asyncParticipantID = userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( PARTICIPANT_ROLE_ID ),
				null,
				"Teilnehmer",
				"SBB Asynchron",
		...
		sessionService.join( new Invitation( null,
				(UserImpl)userService.findByID( asyncParticipantID ),
				(SessionImpl)sessionService.findByID( asyncSessionID ) ) );

		// administrator
		userService.persist( new UserImpl( new PasswordCredentialImpl( "abc" ),
				(RoleImpl)roleService.findByID( ORGANIZER_ROLE_ID ),
				null,
				"Administrator",
				"SBB Workshop Manager",
				"administrator@sbb" ) );
