...
public class P2PTwoDataDao extends ExerciseDataDaoImpl
{
	...
	public List< ExerciseDataImpl > findByExerciseID( String exerciseID )
	{
		List< P2PTwoData > data = getEntityManager().createQuery(
			"select distinct d from P2PTwoData d LEFT JOIN FETCH d.owner LEFT JOIN FETCH d.selectedKeywords where d.workflowElement.id = '" + exerciseID + "' ORDER BY d.timestamp ASC" ).getResultList();
		...
		for ( ExerciseImpl ex : UserContext.getCurrentUser().getSession().getWorkshop().getExercises() )
		{
			if ( ex.getDefinition().getClass().getSimpleName().equalsIgnoreCase( P2PTwoDefinition.class.getSimpleName() ) )
			{
				data.addAll( getEntityManager().createQuery(
					"select distinct d from P2PTwoData d LEFT JOIN FETCH d.owner LEFT JOIN FETCH d.answers LEFT JOIN FETCH d.selectedKeywords where d.workflowElement.id = '" + ex.getID() + "' ORDER BY d.timestamp ASC" ).getResultList() );
			}
		}
